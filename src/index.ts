...
type ChatContext = Pick<ChatMessage, 'parentMessageId' | 'conversationId'>;
...
async function main() {
  ...
  const ownerIdList = process.env.OWNER_ID?.length ? process.env.OWNER_ID?.split(',').map((x) => parseInt(x)) : [];
  const groupIdList = process.env.GROUP_ID?.length ? process.env.GROUP_ID?.split(',').map((x) => parseInt(x)) : [];
  ...
  async function authenticate(msg: TelegramBot.Message) {
    if (msg.chat.type === 'private') {
      if (ownerIdList.length !== 0 && ownerIdList.indexOf(msg.chat.id) === -1) {
        ...
        logWithTime('⚠️ Authentication failed for user ' + `@${msg.from?.username ?? ''} (${msg.from?.id}).`);
        return false;
      }
    } else {
      if (groupIdList.length !== 0 && groupIdList.indexOf(msg.chat.id) === -1) {
        ...
        logWithTime(`⚠️ Authentication failed for group ${msg.chat.title} (${msg.chat.id}).`);
        return false;
      }
    }
    return true;
  ...
  async function commandHandler(msg: TelegramBot.Message, command: string, isMentioned: boolean) {
    const userInfo = `@${msg.from?.username ?? ''} (${msg.from?.id})`;
    const chatInfo = msg.chat.type === 'private' ? 'private chat' : `group ${msg.chat.title} (${msg.chat.id})`;
    if (DEBUG >= 1) {
      logWithTime(`👨‍💻️ User ${userInfo} issued command "${command}" in ${chatInfo} (isMentioned=${isMentioned}).`);
    }
    ...
    switch (command) {
      ...
      case '/reset': {
        ...
        chatContext = {};
        await bot.sendMessage(msg.chat.id, '🔄 The chat thread has been reset. New chat thread started.');
        logWithTime(`🔄 Chat thread reset by ${userInfo}.`);
        break;
      }
      default: {
        await bot.sendMessage(msg.chat.id, '⚠️ Unsupported command. Run /help to see the usage.');
        break;
      }
    }
  }
  ...
  async function chatHandler(msg: TelegramBot.Message, text: string) {
    ...
    try {
      ...
      bot.sendMessage(chatId, "⚠️ Sorry, I'm having trouble connecting to the server, please try again later.");
    }
  }
